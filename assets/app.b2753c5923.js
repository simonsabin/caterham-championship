...
function champStats() {
  ...
  const cells = [
    ...
    ['Leading total', `${leader.gross}<small> pts</small>`],
...
function classGroups() {
  const names = (D.classes || []).filter(c => scoring.some(t => t.cls === c));
  if (names.length < 2) return null;
  const groups = names.map(c => ({ name: c, rows: scoring.filter(t => t.cls === c), split: true }));
  ...
  const rest = scoring.filter(t => !names.includes(t.cls));
...
function champTable(rows, split) {
  ...
  rows.forEach(t => {
    ...
    const pos = split ? t.cls_pos : t.pos;
    ...
    const chip = !t.registered
      ...
      : split ? metaChip(`P${t.pos || '—'} overall`)
...
function standings() {
  ...
  const howToRead = Object.assign(el('p', 'lede'),
    { textContent: `Points scored in every race. An underlined score took the fastest lap of that `
  ...
  if (groups) {
    ...
    p.append(champTable(D.table, false));
  }
...
function runin() {
  ...
  sets.forEach(s => {
    ...
    tb.innerHTML = '<thead><tr><th>Pos</th><th>Driver</th><th class="num">Now (gross)</th>'
      ...
      + s.m.map(d => `<tr class="${d.live ? '' : 'outrow'}"><td class="num mono">${d.rank}</td>`
        + `<td>${esc(d.t.driver)}</td><td class="num mono">${d.t.gross}</td>`
...
function candles(m, bestFloor, titleWord = 'title') {
  ...
  m.forEach((d, i) => {
    ...
    const tipText = `<b>${esc(d.t.driver)}</b> — P${d.rank}, ${d.t.gross} pts<br>`
...
function rules() {
  ...
  left.innerHTML = grads() ? graduatesRules() : `
    ...
    With ${D.roundsTotal - D.roundsRun.length} races still to run the <em>Net</em> column is
    indicative; the order shown follows the gross total, as the official leaderboard does.</p>
    <h3>Penalties</h3>
...
function graduatesRules() {
  ...
  return `
    ...
    the <em>Net</em> column is indicative; the order shown follows the gross total.` : ''}</p>
    <h3>Penalties</h3>
    <p>A penalty that carries Motorsport UK licence points also costs three times that number of
...
/** Registered drivers ranked on gross, ties broken as the tables break them. */
function liveRank(d) {
  const reg = d.table.filter(t => t.registered);
  reg.sort((a, b) => b.gross - a.gross || b.wins - a.wins || b.podiums - a.podiums
    || a.driver.localeCompare(b.driver));
  let pos = 0, last = null;
  reg.forEach((t, i) => {
    if (t.gross !== last) { pos = i + 1; last = t.gross; }
    t.pos = pos;
  });
  (d.classes || []).forEach(cls => {
    let p = 0, l = null;
    reg.filter(t => t.cls === cls).forEach((t, i) => {
      if (t.gross !== l) { p = i + 1; l = t.gross; }
      t.cls_pos = p;
    });
  });
