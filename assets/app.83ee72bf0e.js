...
function cellTip(t, rd) {
  ...
  // The zero that has to be carried. Reg 1.6.2 lets a driver discard a
  // disqualification for causing a collision like any other bad round, and not
  // one handed down at scrutineering, so which it was is worth saying here.
  if (undroppable(t, Number(rd)))
    bits.push('Disqualified for a technical infringement, so this round '
      + '<b>cannot be dropped</b> — reg 1.6.2');
...
/* ------------------------------------------------------------- ranking --
   ...
   The championship is decided on the total after the drop scores - reg 1.6.2 -
      ...
      th.dataset.tip = on
        ? `<b>Ranked on ${h}</b><br>` + (by === 'net'
          ? `The total after the drop scores, which is what decides the championship `
            + `<span class="reg">${D.dropComponent === 'race_points' ? '1.6.2a' : '1.6.2'}</span>.`
 ...
 * it was set in. A `null` there is a round no drop can reach at all - reg 1.6.2
 * makes a driver excluded from a result for a technical infringement carry that
 * zero - so it is out of the candidates rather than first among them.
 * `penalty` is already negative, and is applied after the drop rather than
 * inside a round, so a drop can never cancel one. `bonus` is points awarded
 * outside the races at all - Academy's marshalling and briefings.
 *
...
function seasonTotal(scores, droppable, dropScores, penalty, bonus) {
  ...
  const sorted = droppable.filter(v => v !== null).sort((a, b) => a - b);
...
/** The rounds reg 1.6.2 will not let this driver drop, as build_all.py found them. */
const undroppable = (t, r) => (t.undroppable || []).indexOf(r) >= 0;
...
function roundScores(t, rounds, dropComponent) {
  ...
  rounds.forEach(r => {
    ...
    droppable.push(undroppable(t, r) ? null : (s ? s[part] : 0));
  });
...
function project(t, v, d, open) {
  ...
  for (let r = 1; r <= d.roundsTotal; r++) {
    if (run.has(r)) {
      ...
      droppable.push(undroppable(t, r) ? null
                     : x ? (partOnly ? x.race_points : x.score) : 0);
    } else {
...
function rules() {
  ...
  left.innerHTML = grads() ? graduatesRules() : `
    ...
    <span class="reg">1.6.1</span>. A lap on which the driver exceeded track limits cannot count, and
    ...
    <h3>Drop scores</h3>
    <p>${D.dropScores === 1 ? 'The lowest round score is' : `The ${D.dropScores} lowest round
    scores are`} discarded from the season total <span class="reg">1.6.2</span>. A round score is race points plus the fastest lap point, and a
    round a driver did not contest counts as a zero and is a candidate for dropping.
    A round a driver was <em>disqualified from for a technical infringement</em> is not: the same
    article makes them carry that zero to the end of the season, where a disqualification for
    causing a collision is an ordinary zero and can be dropped like any other.
 ...
 * the fastest lap point (reg 1.6.1) goes to the quickest points-scoring car
...
function liveScore(base, rows, session, round) {
  ...
  // scoring car's - which is what reg 1.6.1 does with a guest's.
  let flDriver = null;
...
function liveNetGain(d, t, key) {
  ...
  d.roundsRun.forEach(r => {
    ...
    // Reg 1.6.2 will not let a technical disqualification be dropped, so that
    // round is out of the candidates on both sides of this subtraction - which
    // is what makes every other round worth less to this driver: the drop it
    // displaces now falls on a round they scored in rather than on that zero.
    const keep = undroppable(t, r);
    scored.scores.push(s ? s.score : 0);
    scored.droppable.push(keep ? null : s ? s[part] : 0);
    blank.scores.push(s && !mine ? s.score : 0);
    blank.droppable.push(keep ? null : s && !mine ? s[part] : 0);
  });
